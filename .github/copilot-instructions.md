...
This adapter connects ioBroker to Network UPS Tools (NUT) servers to monitor Uninterruptible Power Supply (UPS) devices. NUT is a network-based framework that provides a common interface to monitor and manage UPS devices across different manufacturers.

**Adapter Purpose:**
- Monitor UPS status, battery levels, and power conditions
- Provide real-time notifications for power events (battery mode, low battery, power restored)
- Enable integration of UPS data into smart home automation workflows
- Support remote monitoring of UPS devices connected to various systems

**Target Systems:**
- NUT servers running on Linux systems, NAS devices (Synology, QNAP), and dedicated UPS management systems
- UPS devices from various manufacturers (APC, Eaton, CyberPower, etc.) compatible with NUT
- Network-accessible UPS monitoring solutions

**Key Configuration Parameters:**
- `host_ip`: IP address of the NUT server (localhost, LAN IP, or remote IP)
- `host_port`: NUT server port (default: 3493)
- `ups_name`: UPS identifier as defined in NUT configuration ("ups" for Synology, "qnapups" for QNAP)
- `update_interval`: Data polling frequency in seconds (default: 300s)
- `username`/`password`: Authentication credentials for protected NUT servers

**Protocol Specifics:**
- Uses node-nut library for NUT protocol communication
...
### NUT Adapter Specific Patterns

**UPS Status Management:**
- Use severity levels (idle=0, operating=1, operating_critical=2, action_needed=3, unknown=4) for UPS status classification
- Implement status flags (online, battery, lowbattery, replacebattery, etc.) as boolean states
- Handle UPS variables consistently with NUT naming conventions (battery.charge, ups.status, etc.)

**Connection Handling:**
- Implement robust error handling for network timeouts and NUT server unavailability
- Use connection retry logic with exponential backoff for temporary network issues
- Delay adapter initialization when UPS is not reachable on startup (as implemented in v1.6.0)
- Maintain connection state monitoring and automatic reconnection

**Data Processing:**
- Parse NUT variable responses correctly (handle space-separated values, quoted strings)
- Convert UPS data types appropriately (numbers, booleans, strings) for ioBroker states
- Implement proper units and value ranges for battery levels, voltages, and power metrics
- Handle device-specific variations in NUT variable naming and values

**State Management:**
- Create hierarchical state structure: info.*, status.*, battery.*, input.*, output.*
- Implement proper state roles and types for UPS data (level.battery, switch.power, etc.)
- Use acknowledged states for UPS data and unacknowledged for commands
- Maintain compatibility with existing state structure for upgrades

**Command Support:**
- Implement UPS command execution (shutdown, test.battery.start, etc.) with proper validation
- Handle command responses and errors appropriately
- Ensure commands are sent only when UPS supports them (check available commands)

**Error Handling:**
- Log connection issues as warnings, not errors, when UPS is temporarily unavailable
- Implement graceful degradation when partial UPS data is available
- Handle NUT server authentication failures with clear error messages
- Use Sentry error reporting appropriately (exclude expected connection issues)